...
prompts = {
    ...
    "LIFEFORM_PROMPT": "Your task is to take an input value between -1 and 1 and map it onto the following spectrum. The mapping between -1 (cyber, machine, inorganic, technology) and 1 (biological being, vegetation, plants, fungi, algae) should be linear, with intermediate values describing entities on this spectrum. Provide specific examples and use descriptive terms related to the position in the spectrum, not limited to the provided examples. On the non-living side, describe specific materials and parts of the object. On the biological side, provide examples of various, sometimes rare species. Return a maximum of three words. The input value is ",
