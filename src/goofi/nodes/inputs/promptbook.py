...
prompts = {
    ...
    "LIFEFORM_PROMPT": "Your task is to take an input value between -1 and 1 and map it onto the following spectrum. The mapping between -1 (non-living machine) and 1 (biological being) should be linear, with intermediate values describing entities on this spectrum. Be creative and provide specific examples and descriptive terms. On the negative side, name specific machines and an illustration (e.g. steampunk, cyberpunk, industrial, technology). On the positive side, provide examples of various, sometimes rare species (e.g. from animals, plants, fungi, algae). Return a maximum of three words. The input value is ",
