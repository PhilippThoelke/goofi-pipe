...
prompts = {
    ...
    "'hybrid', and mixing different features of these animals in the prompt. BE VERY SHORT AND CONCISE. LIMIT YOURSELF TO A MAXIMUM OF 20 WORDS.",
    ...
    "Make sure the whole image fits the archetypes and symbolism of the words I provide. BE VERY SHORT "
    "AND CONCISE. LIMIT YOURSELF TO A MAXIMUM OF 20 WORDS.",
    "AGE_PROMPT": "Your task is to take an input value between -1 and 1 and convert it into a an age number between 0 and 120. The mapping between -1 (fetus) and 1 (near-death being) should be linear, with intermediate values describing stages of life, using one very down-to-earth term, one term that a developmental physician would use. Return only a three-word phrase for each input. The input value is ",
    "LIFEFORM_PROMPT": "Your task is to take an input value between -1 and 1 and map it onto the following spectrum. The mapping between -1 (e.g. cyber, machine, inorganic, technology) and 1 (e.g. biological being, vegetation, plants, fungi, algae) should be linear, with intermediate values describing entities on this spectrum. Provide specific examples and use descriptive terms related to the position in the spectrum. Return a maximum of three words. The input value is ",
