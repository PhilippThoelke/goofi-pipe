...
class DissonanceCurve(Node):
    ...
    def config_output_slots():
        return {"dissonance_curve": DataType.ARRAY,
                "tuning": DataType.ARRAY,
                "avg_dissonance": DataType.ARRAY}

    def config_params():
        return {
            "Dissonance_Curve": {
                "max_ratio": FloatParam(2.0, 1.0, 10.0, doc="Maximum ratio to extend the dissonance curve to; 2 represents the octave"),
    ...
    def process(self, peaks: Data, amps: Data):
        ...
        if peaks.data.ndim == 1:   
            ...
            return {"dissonance_curve": (np.array(diss), metadata),
