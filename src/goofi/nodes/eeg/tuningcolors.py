...
class TuningColors(Node):
    ...
    def config_output_slots():
        return {"hue": DataType.ARRAY,
                 "saturation": DataType.ARRAY,
    ...
    def process(self, data: Data):
        ...
        hsvs = viz_scale_colors(data.data, fund=data.data[0])[1:]

        return {"hue": (np.array([x[0] for x in hsvs]), data.meta),
                "saturation": (np.array([x[1] for x in hsvs]), data.meta),
                "value": (np.array([x[2] for x in hsvs]), data.meta)}



def viz_scale_colors(scale, fund):
    """
